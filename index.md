...
I have designed, developed, and delivered quality oral, written, and visual communications by convertint the project to a Python application, creating a nice GUI to accompany it (I updated the GUI to show the insert a bid fields only when "insert a bid" is selected), adding multiple sorting algorithms, and applying error handling protocols into the code. The application sorts by parameters and implements bubble sort in one of the sorting options as well. Adding a database that implemented the attributes in the proper columns was also important. I then added unit tests and error handling for insert a bid attempts to reject bad entries. I included a README file in the repository stored final project to explain changes. Doing all of these changes met Outcome 2.

### *Outcome 3*

I designed and evaluated computing solutions that solved given problems using algorithmic principles by implementing sort by parameter functions within the code. Users can now sort through the data provided within the CSV file and the database and view how long they took and which sorting algorithms were used. I also met this course outcome by recreating each algorithm and function in a Python application rather than C++ which showed my ability to translate code to a different language. I also implemented bubble sort as an additional algorithm for sort by title to demonstrate a algorithm that wasn't in the pandas library. I've made sure to keep computer science practices and standards in mind as a priority when creating each function, variable, and organizing the program, therefore all is named properly. The design is a lot nicer using a GUI instead of commandline for the application. I cleaned up the GUI as much as possible, commented out the code, added error handling parameters, and made it all come together.

### *Outcome 4*

I demonstrated an ability to use well-founded and innovative techniques, skills, and tools while also meeting industry-specific goals meet this course by creating the classes to modularize the code, utilizing sort functions by parameters, and setting up the GUI using event handlers for the radio select buttons. Implementing algorithms and modularizing the code ensured I am meeting industry-specific goals as this is a very common implementation in the industry. Another addition that was industry specific was implementing error handling into the code, since it now throws errors if attempts to inject improper entries are made. That had to be done because before, a user could enter any number or letter for any of the parameters and break the sorting algorithm functionality. I also made sure to comment out changes and create a readme file to meet this outcome further.

### *Outcome 5*

I made sure to meet this course outcome by hosting the database locally so that the code is automatically secure. I have also commented out every part of the code and made sure deallocation is in place. The code now checks the user entries for type, which ensures that not just anything can be entered into the database which would break the program. This gave the program another light level of security, future plans are to recreate the database functionality in full to encrypt the data or add a username and password.
